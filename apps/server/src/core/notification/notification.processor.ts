  ...
  async process(
    ...
    try {
      ...
      switch (job.name) {
        ...
        case QueueJob.PAGE_PERMISSION_GRANTED_NOTIFICATION: {
          await this.pageNotificationService.processPagePermissionGranted(
            job.data as IPagePermissionGrantedNotificationJob,
            appUrl,
          );
          break;
        }
