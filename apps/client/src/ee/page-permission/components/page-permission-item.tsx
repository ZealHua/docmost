      ...
      <div className={classes.permissionItemRole}>
        {isCurrentUser || disabled ? (
          <Text size="sm" c="dimmed">
            {t(roleLabel || PagePermissionRole.READER)}
          </Text>
        ) : (
          <Menu withArrow position="bottom-end">
            <Menu.Target>
              <UnstyledButton>
                <Group gap={4}>
                  <Text size="sm">{t(roleLabel || PagePermissionRole.READER)}</Text>
            ...
            <Menu.Dropdown>
              ...
              <Menu.Divider />
              <Menu.Item color="red" onClick={() => onRemove(member.id, member.type)}>
                {t("Remove access")}
              </Menu.Item>
            </Menu.Dropdown>
          </Menu>
