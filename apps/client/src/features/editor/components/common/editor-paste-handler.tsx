...
async function reuploadPastedAttachments(
  ...
  editor.state.doc.nodesBetween(pasteFrom, pasteEnd, (node, pos) => {
    ...
    const fileName = node.attrs.name || src.split("/").pop() || "file";
  ...
  const nodesToReupload = pastedNodes.filter((node) => {
    const ownerPageId = attachmentPageMap.get(node.attachmentId);
  ...
  await Promise.all(
    [...uniqueNodes.values()].map(async (node) => {
      ...
      try {
        ...
        const file = new File([blob], node.fileName ?? "file", {
          type: blob.type,
        });
