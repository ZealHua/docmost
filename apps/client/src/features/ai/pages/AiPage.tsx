...
export default function AiPage() {
  ...
  const { stopStream, clearMessages, setSession, activeSession } =
  ...
  const restoreSelectedPages = async (selectedPageIds: string[]) => {
    ...
    try {
      const pages = await pageSearch.mutateAsync({
  ...
  useEffect(() => {
    ...
    if (
      urlSessionId &&
      urlSessionId !== restoredSessionRef.current &&
      !sessionsLoading
    ) {
  ...
  useEffect(() => {
    if (
      activeSession?.id &&
      !activeSession.id.startsWith("local-") &&
      activeSession.id !== urlSessionId
    ) {
      // Prevent the URL restore effect from wiping out our newly created session state
      restoredSessionRef.current = activeSession.id;
      navigateToSession(activeSession.id);
    }
  }, [activeSession?.id, urlSessionId, navigateToSession]);
  ...
  const handleSelectSession = (session: AiSession, skipNavigate = false) => {
    restoredSessionRef.current = session.id;
    setSession(session);
    clearMessages();
